...
var ChatBot = function(username, password, options) {
	...
	this.steamUser = new Steam.SteamUser(steamClient);
	this.steamFriends = new Steam.SteamFriends(steamClient);
	steamClient.connect();
	this.version = ver;
		...
		try {
			if(this.guardCode) {
				this.steamUser.logOn({
					...
					sha_sentryfile: (fs.existsSync(this.sentryFile) ? fs.readFileSync(this.sentryFile) : undefined)
				});
			} else {
